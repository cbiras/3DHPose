...
class MemDataset ( Dataset ):
    ...
    def __init__(self, info_dict, camera_parameter=None, template_name='Shelf'):
        ...
        #asta e normalizarea ca sa fie in (0,1). formula e img = (img - mean)/std. Sunt 3 fiindca se face normalizare pe fiecare channel(red,green,blue)
        self.normalizer = T.Normalize ( mean=[0.485, 0.456, 0.406],
                                        std=[0.229, 0.224, 0.225] )
        #merg in transforms sa vad ce se intampla
        #ceva e dubios ca nu gasesc functiile
        ...
        #face un dictionar de genul (img_id,tensor(dimensiunea numarului de persoane))
        self.dimGroup = OrderedDict ()

        #ar fi foarte frumos daca as stii ce e in info_dict. ideea e ca se parcurg imaginile in ordine, da doar pt prima camera ca probabil e acelasi lucru
        for img_id in [i for i in self.info_dict[self.cam_names[0]].keys () if i != 'image_data' and i != 'image_path']:
            ...
            for cam_id in self.cam_names:
                #se calculeaza numarul de persoane ca si cate imagini cropuite sunt in dictionarul unei anumite iamgini, de la o anumita camera
                #are sens, nu fi retard
                num_person = len ( self.info_dict[cam_id][img_id] )
                #pentru fuecare camera noua, se tot aduna numarul de persoane din imagine
                cnt += num_person
                this_dim.append ( cnt )
            #pentru ca asta e in afara for-ului, nu e dubios.
            #fiecare indice din dimGroup reprezinta toate 3 camerele de la indicele ala
            #in dimGroup sunt tensori cu numarul de persoane adunate per fiecare grup de imagini(3)
        ...
        #nu inteleg ce face torch.inverse si aproape nimic din codul de mai jos. ACUM CITESC CAM-STYLE SI SPER A MA LAMURI
        #edit peste 2 saptamani de la comentariul original. tot nu stiu care e treaba cu parametrii camerelor da nici nu m-am mai chinuit
    ...
    def __getitem__(self, item):
        ...
        Practic asta e folosita pentru rezultatele vizuale
        :param item:
    ...
    def get_unary(self, person, sub_imgid2cam, candidates, img_id):
        ...
        info_list = list ()  # This also occur in multi estimator
        for cam_id in self.cam_names:
