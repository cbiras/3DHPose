...
def matchSVT(S, dimGroup, **kwargs):
    #asta cred ca e algoritmul de gasire a matricii P
    ...
    info = dict ()
    #trebuie sa aflu ce e la W shape[0]
    N = S.shape[0]
    #de la range de N la range de N, S-ul ia valoarea 0
    S[torch.arange ( N ), torch.arange ( N )] = 0
    #S.t() inseamna ca se face transpusa lui S
    S = (S + S.t ()) / 2
    X = S.clone ()
    Y = torch.zeros_like ( S )
    W = alpha - S
    t0 = time.time ()
    #se fac 500 de iteratii
    for iter_ in range ( maxIter ):
        #X-ul e o clona de a lui S, care a fost procesat cumva. Joaca practic rolul matricii de afinitate
        X0 = X
        # update Q with SVT
        #practic U,s si v sunt Q in primul algoritm
        U, s, V = torch.svd ( 1.0 / mu * Y + X )
        diagS = s - _lambda / mu
        #tot ce e mai mic decat 0 se normalizeaza la 0
        diagS[diagS < 0] = 0
        Q = U @ diagS.diag () @ V.t ()
        # update X
        X = Q - (W + Y) / mu
        # project X
        #nu stiu exact ce inseamna sa proiecteze matricea de permutare
