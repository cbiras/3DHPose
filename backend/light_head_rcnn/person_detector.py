...
class PersonDetector ( object ):
    ...
    def detect(self, image, image_id):
        """
        detect on a single image.-SCRIE CA ON A SINGLE IMAGE, SUNT PROST RAU
        #zice ca parametrul trebuie sa fie path, dar stiam ca is ceva tensori. Trebuie sa vad cum vizualizez ce e aici
    ...
    #ia ceva variabile din checkpoints si le restore-uie, da nu am inteles mai mult de atat
    def _load_model(self, model_file):
        from tensorflow.python import pywrap_tensorflow
        def get_variables_in_checkpoint_file(file_name):
            try:
                #asa cum zice si numele functiei, aici citeste variabilele din fisierul de checkpoints
                reader = pywrap_tensorflow.NewCheckpointReader ( file_name )
                #trebuie sa ma uit in documentatie la ce e aia shape_map()
        ...
        #e un dictionar cu tot ce s-a obtinut in urma inferentei, cls_score, cls_prob. CLs_prob e cls_score trecut printr-un softmax, bbox si rois
        ...
        #aici zice ceva de tf_cpn dar nu vad nici o trimitere spre vreo functie din tf_cpn
